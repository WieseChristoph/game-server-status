            ...
            >
              <View className="bg-[#2f333f] p-2 shadow-lg shadow-black rounded-md mx-4 mb-4 flex flex-row items-center min-h-[125px] relative">
                ...
                {server.type === ServerType.Minecraft ? (
                  (server.data as MinecraftServer)?.favicon ? (
                    // Server icon
                    <Image
                      style={{ width: 48, height: 48 }}
                      source={{ uri: (server.data as MinecraftServer)?.favicon }}
                    />
                  ) : (
                    // Minecraft icon
...
const EditServerButton: React.FC<{ onEditPress: () => void }> = ({ onEditPress }) => {
  return (
    <Pressable onPress={onEditPress}>
      <View className="bg-yellow-500 p-2 rounded-md ml-4 mb-4 flex flex-1 justify-center">
        <Svg
          width="32"
          height="32"
          viewBox="0 0 24 24"
...
const DeleteServerButton: React.FC<{ onDeletePress: () => void }> = ({ onDeletePress }) => {
  return (
    <Pressable onPress={onDeletePress}>
      <View className="bg-red-500 p-2 rounded-md mr-4 mb-4 flex flex-1 justify-center">
        <Svg
          width="32"
          height="32"
          viewBox="0 0 24 24"
