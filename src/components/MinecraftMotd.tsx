      ...
      return (
        <Text
          ...
          style={{
            ...
            textDecorationLine: underlined ? (strikethrough ? "underline line-through" : "underline") : "none",
          }}
        >
          {part}
        </Text>
...
const MinecraftMotd: React.FC<{ motd: string | MinecraftServerDescription }> = ({ motd }) => {
  if (typeof motd === "string" || motd.extra === undefined)
    return <Text style={{ color: "#FFFFFF" }}>{motdStringToJsx(typeof motd === "string" ? motd : motd.text)}</Text>;
  else
    return (
      <>
